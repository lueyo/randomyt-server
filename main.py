...
async def get_random_video(
    params: RandomByDayQueryParamsDTO = Depends(),
    videoService: IVideoService = Depends(get_video_service),
):
    """
    ...
    - **params**: DTO containing day or startDay/endDay for filtering.
    - **videoService**: Dependency-injected service for handling video operations.

    Returns:
    - A VideoSchema object containing the video details.
    """
    video = await videoService.get_random_video_by_dto(params)
...
async def get_random_video_exclude_ids(
    ...
    - A VideoSchema object containing the video details.
    """
    # Determinar si usar día específico o intervalo
    if day:
        params = RandomByDayQueryParamsDTO(day=day)
    else:
        # Set default endDay to today if not provided
        effective_endDay = endDay if endDay else datetime.now().strftime("%d/%m/%Y")
        params = RandomByIntervalQueryParamsDTO(
            startDay=startDay, endDay=effective_endDay
        )

    video = await videoService.get_random_video_by_dto_exclude_ids(params, request.ids)
...
async def search_by_day(
    params: SearchDayQueryParamsDTO = Depends(),
    videoService: IVideoService = Depends(get_video_service),
):
    """
    ...
    - **params**: DTO containing day, page, pageSize, and sort parameters.
    - **videoService**: Dependency-injected service for handling video operations.
    ...
    """
    try:
        result = await videoService.search_by_day_dto(params)
        return result
...
async def search_by_interval(
    params: SearchIntervalQueryParamsDTO = Depends(),
    videoService: IVideoService = Depends(get_video_service),
):
    """
    ...
    - **params**: DTO containing startDay, endDay, page, pageSize, and sort parameters.
    - **videoService**: Dependency-injected service for handling video operations.
    ...
    """
    try:
        result = await videoService.search_by_interval_dto(params)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
